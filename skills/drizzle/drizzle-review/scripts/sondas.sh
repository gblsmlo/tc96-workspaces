...
"${RG[@]}" -U 'drizzle\((?s:.{0,300}?)schema' "$DIR" | head -5 || echo "   NÃO ENCONTRADO — sem { schema }, db.query.x é undefined"
...
"${RG[@]}" -o 'index\(([^)]*)\)\.on\(([^)]*)\)' "$DIR" | head -20 || vazio
...
"${RG[@]}" -l 'z\.object\(' "$DIR" 2>/dev/null | head -5 || vazio
