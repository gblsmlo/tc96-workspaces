---
name: elysia-diagnose
description: Diagnosticar por que um plugin ou hook não afeta uma rota em Elysia, e escolher o ponto certo do lifecycle — ordem de registro, escopo `local`/`scoped`/`global`, `name` de plugin, `derive` × `resolve`, `state` × `decorate`, macro, tracing — citando IDs `ELYSIA-LIFE-*`, com dez sondas executáveis — use quando a tarefa for investigar hook que não roda, autenticação que não protege a rota do consumidor, valor do `store` congelado, plugin cujo lifecycle roda uma vez só, ou span `anonymous` no OpenTelemetry. Não use para escrever rota e handler, que é elysia-build, nem para schema e Eden, que é elysia-schema.
tags:
  - skill
  - elysia
  - backend
fonte: "[[Elysia - Lifecycle e Plugins]]"
---

# elysia-diagnose

> **Fonte desta skill:** [[Elysia - Lifecycle e Plugins]], com o hub [[Elysia]] como roteador.
...
| --- | --- | --- |
| 1 | [[Elysia]] § 2 | o modelo mental de instância e plugin |
| 2 | [[Elysia]] § 6 + § 6.1 + § 6.2 | regras, críticas e IDs canônicos |
| 3 | [[Elysia - Lifecycle e Plugins]] | a fonte |
...
```bash
bash ~/.claude/skills/elysia-diagnose/scripts/sondas.sh src
```
...
- [[Elysia - Lifecycle e Plugins]] — fonte desta skill
- [[Elysia]] § 6, § 7
