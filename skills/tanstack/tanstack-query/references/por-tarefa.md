...
| --- | --- |
| 1. Ler dado remoto pela primeira vez | [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/docs/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) → [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) |
| 2. Decidir política de frescor | [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) |
| 3. Escrever no servidor e invalidar | [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/docs/tanstack-query-mutations-e-invalidacao.md) |
| 4. Update otimista com rollback | [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/docs/tanstack-query-mutations-e-invalidacao.md) → [React - Formulários e Actions](../../../../knowledge-base/docs/react-formularios-e-actions.md) § 5 |
| 5. Lista paginada ou infinita | [TanStack Query - Padrões de Consulta](../../../../knowledge-base/docs/tanstack-query-padroes-de-consulta.md) → [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) |
| 6. Integrar com o loader de rota | [TanStack Router - Carregamento de Dados](../../../../knowledge-base/docs/tanstack-router-carregamento-de-dados.md) § 8 → [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/docs/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) § 7 |
| 7. Diagnosticar cache | ver a seção **Diagnóstico** — o satélite sai do sintoma |

Suspense, SSR/hidratação, cancelamento e network mode ficam em [TanStack Query - Suspense e SSR](../../../../knowledge-base/docs/tanstack-query-suspense-e-ssr.md) e aparecem como ramo de outras tarefas, não como tarefa de entrada.
...
**Carregar:** [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/docs/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) (§ 3 keys, § 4 query functions, § 5 `status` × `fetchStatus`, § 7 `queryOptions`); [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) só quando chegar na política de frescor — que é a tarefa 2 e **não é opcional**.

**Verificar:**

- O `QueryClient` está fora do corpo do render? → `TSQ-BASE-01`
- A key é array, serializável e única para aquele dado? → `TSQ-BASE-02`
- **Toda** variável lida pela `queryFn` está na key? É a causa raiz da maioria dos bugs de dado velho ([TanStack Query](../../../../knowledge-base/docs/tanstack-query.md) § 2, ponto 3) → `TSQ-BASE-03`
...
- A resposta passa por schema antes de virar `data`? A fronteira HTTP valida ([TanStack Query](../../../../knowledge-base/docs/tanstack-query.md) § 7, invariante 5;) → `TSQ-BASE-06`
...
**Não faça:** guardar o resultado em `useState`, nem "salvar a resposta" em qualquer lugar. Isso cria a segunda fonte de verdade que diverge na primeira revalidação — `REACT-PAT-03` em [React - Patterns](../../../../knowledge-base/docs/react-patterns.md), e invariante 1 do contrato. Se o código atual busca em `useEffect`, o achado é `REACT-EFFECT-06` ([React - Efeitos e Sincronização](../../../../knowledge-base/docs/react-efeitos-e-sincronizacao.md)), e a correção é a query, não um Effect melhor.

### 2. Decidir política de frescor (`staleTime` / `gcTime`)

**Carregar:** [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) (§ 2 ciclo de vida, § 3 `staleTime`, § 4 gatilhos de refetch).

Percorra a árvore "Qual `staleTime`?" de [TanStack Query](../../../../knowledge-base/docs/tanstack-query.md) § 5 antes de escrever o número. A calibração se faz pela **volatilidade do dado**, não pela tela.
...
- `staleTime` e `gcTime` governam fases diferentes (ativa × inativa) e foram decididos separadamente? A combinação alto/baixo é a pior possível — ver [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) § 2
...
**Carregar:** [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/docs/tanstack-query-mutations-e-invalidacao.md) (§ 2 `useMutation`, § 3 `invalidateQueries`, § 4 `setQueryData`).

Percorra a árvore "Escrevi no servidor. E agora?" de [TanStack Query](../../../../knowledge-base/docs/tanstack-query.md) § 5. **Escrita não atualiza a tela — invalidação atualiza** ([TanStack Query](../../../../knowledge-base/docs/tanstack-query.md) § 2, ponto 5). Nenhuma mutation sai desta skill sem responder o que ela tornou velho, ou justificar por que nada ficou velho (invariante 3).
...
- Falha esperada (409, validação, sem permissão) vira estado da UI; falha inesperada vai para o boundary → `REACT-ASYNC-09` em [React - Suspense e Assincronia](../../../../knowledge-base/docs/react-suspense-e-assincronia.md)
- **A assinatura dos callbacks está na forma atual da v5** — ver a seção seguinte. Confira sempre, não por memória.

**Calibre a dose:** invalidar de menos deixa tela velha; invalidar de mais transforma cada escrita numa cascata de requests. O `refetchType` (default `'active'`) é o que quase ninguém configura e o que decide o tamanho da rajada — [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/docs/tanstack-query-mutations-e-invalidacao.md) § 3.

### 4. Update otimista com rollback

**Carregar:** [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/docs/tanstack-query-mutations-e-invalidacao.md) § 5 (ciclo completo e a alternativa por `variables`); [React - Formulários e Actions](../../../../knowledge-base/docs/react-formularios-e-actions.md) § 5 **antes de escolher a camada**.
...
**Carregar:** [TanStack Query - Padrões de Consulta](../../../../knowledge-base/docs/tanstack-query-padroes-de-consulta.md) (§ 3 paginadas, § 4 infinitas); [TanStack Query - Cache e Frescor](../../../../knowledge-base/docs/tanstack-query-cache-e-frescor.md) § 6 para `placeholderData` × `initialData`.

Antes: a página/cursor pertence à URL? Se o usuário precisa compartilhar link ou usar o botão voltar, é search param — `REACT-PAT-10`, e a rota entra pela `tanstack-router`. Modelo de paginação em.
...
**Carregar:** [TanStack Router - Carregamento de Dados](../../../../knowledge-base/docs/tanstack-router-carregamento-de-dados.md) § 8 ("External data loading"), que traz o critério e o padrão; depois [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/docs/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) § 7 para o `queryOptions` compartilhado. [TanStack Router - Route Context e Code Splitting](../../../../knowledge-base/docs/tanstack-router-route-context-e-code-splitting.md) § 2 quando o `queryClient` precisar chegar ao loader por contexto.
...
Se a tela usa `useSuspenseQuery`, SSR ou hidratação, aí sim abra [TanStack Query - Suspense e SSR](../../../../knowledge-base/docs/tanstack-query-suspense-e-ssr.md) — em especial `TSQ-SSR-04` (`QueryClient` por requisição no servidor, ou o cache vaza dado entre usuários), `TSQ-SSR-05` (`staleTime` > 0 no cliente, ou a hidratação descarta o que o servidor buscou) e `TSQ-SSR-07` (`<HydrationBoundary>`).
