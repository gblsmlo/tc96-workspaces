...
| --- | --- |
| 1. Reading remote data for the first time | [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) → [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) |
| 2. Deciding the freshness policy | [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) |
| 3. Writing to the server and invalidating | [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/tanstack-query-mutations-e-invalidacao.md) |
| 4. Optimistic update with rollback | [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/tanstack-query-mutations-e-invalidacao.md) → [React - Formulários e Actions](../../../../knowledge-base/react-formularios-e-actions.md) § 5 |
| 5. Paginated or infinite list | [TanStack Query - Padrões de Consulta](../../../../knowledge-base/tanstack-query-padroes-de-consulta.md) → [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) |
| 6. Integrating with the route loader | [TanStack Router - Carregamento de Dados](../../../../knowledge-base/tanstack-router-carregamento-de-dados.md) § 8 → [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) § 7 |
| 7. Diagnosing the cache | see the **Diagnosis** section — the satellite follows from the symptom |

Suspense, SSR/hydration, cancellation and network mode live in [TanStack Query - Suspense e SSR](../../../../knowledge-base/tanstack-query-suspense-e-ssr.md) and appear as a branch of other tasks, not as an entry task.
...
**Load:** [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) (§ 3 keys, § 4 query functions, § 5 `status` × `fetchStatus`, § 7 `queryOptions`); [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) only when you reach the freshness policy — which is task 2 and **is not optional**.
...
- Is **every** variable read by the `queryFn` in the key? It is the root cause of most stale-data bugs ([TanStack Query](../../../../knowledge-base/tanstack-query.md) § 2, point 3) → `TSQ-BASE-03`
...
- Does the response pass through a schema before becoming `data`? The HTTP boundary validates ([TanStack Query](../../../../knowledge-base/tanstack-query.md) § 7, invariant 5;) → `TSQ-BASE-06`
...
**Do not:** store the result in `useState`, nor "save the response" anywhere. That creates the second source of truth that diverges on the first revalidation — `REACT-PAT-03` in [React - Patterns](../../../../knowledge-base/react-patterns.md), and invariant 1 of the contract. If the current code fetches in a `useEffect`, the finding is `REACT-EFFECT-06` ([React - Efeitos e Sincronização](../../../../knowledge-base/react-efeitos-e-sincronizacao.md)), and the fix is the query, not a better Effect.

### 2. Deciding the freshness policy (`staleTime` / `gcTime`)

**Load:** [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) (§ 2 lifecycle, § 3 `staleTime`, § 4 refetch triggers).

Walk the "Qual `staleTime`?" tree in [TanStack Query](../../../../knowledge-base/tanstack-query.md) § 5 before writing the number. Calibration follows the **data's volatility**, not the screen.
...
- Do `staleTime` and `gcTime` govern different phases (active × inactive) and were they decided separately? The high/low combination is the worst possible one — see [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) § 2
...
**Load:** [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/tanstack-query-mutations-e-invalidacao.md) (§ 2 `useMutation`, § 3 `invalidateQueries`, § 4 `setQueryData`).

Walk the "Escrevi no servidor. E agora?" tree in [TanStack Query](../../../../knowledge-base/tanstack-query.md) § 5. **A write does not update the screen — invalidation does** ([TanStack Query](../../../../knowledge-base/tanstack-query.md) § 2, point 5). No mutation leaves this skill without answering what it made stale, or justifying why nothing became stale (invariant 3).
...
- Does an expected failure (409, validation, no permission) become UI state, while an unexpected one goes to the boundary → `REACT-ASYNC-09` in [React - Suspense e Assincronia](../../../../knowledge-base/react-suspense-e-assincronia.md)
- **Are the callback signatures in v5's current form** — see the next section. Always check, never from memory.

**Calibrate the dose:** invalidating too little leaves a stale screen; invalidating too much turns every write into a cascade of requests. `refetchType` (default `'active'`) is what almost nobody configures and what decides the size of the burst — [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/tanstack-query-mutations-e-invalidacao.md) § 3.

### 4. Optimistic update with rollback

**Load:** [TanStack Query - Mutations e Invalidação](../../../../knowledge-base/tanstack-query-mutations-e-invalidacao.md) § 5 (the complete cycle and the `variables` alternative); [React - Formulários e Actions](../../../../knowledge-base/react-formularios-e-actions.md) § 5 **before choosing the layer**.
...
**Load:** [TanStack Query - Padrões de Consulta](../../../../knowledge-base/tanstack-query-padroes-de-consulta.md) (§ 3 paginated, § 4 infinite); [TanStack Query - Cache e Frescor](../../../../knowledge-base/tanstack-query-cache-e-frescor.md) § 6 for `placeholderData` × `initialData`.
...
**Load:** [TanStack Router - Carregamento de Dados](../../../../knowledge-base/tanstack-router-carregamento-de-dados.md) § 8 ("External data loading"), which brings the criterion and the pattern; then [TanStack Query - O que um Dev Frontend Precisa Saber](../../../../knowledge-base/tanstack-query-o-que-um-dev-frontend-precisa-saber.md) § 7 for the shared `queryOptions`. [TanStack Router - Route Context e Code Splitting](../../../../knowledge-base/tanstack-router-route-context-e-code-splitting.md) § 2 when the `queryClient` has to reach the loader through context.
...
If the screen uses `useSuspenseQuery`, SSR or hydration, then open [TanStack Query - Suspense e SSR](../../../../knowledge-base/tanstack-query-suspense-e-ssr.md) — especially `TSQ-SSR-04` (a `QueryClient` per request on the server, or the cache leaks data between users), `TSQ-SSR-05` (`staleTime` > 0 on the client, or hydration discards what the server fetched) and `TSQ-SSR-07` (`<HydrationBoundary>`).
