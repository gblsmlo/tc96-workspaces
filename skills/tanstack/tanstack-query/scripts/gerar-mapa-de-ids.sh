#!/usr/bin/env bash
# Regenerates references/mapa-de-ids.md from knowledge-base/docs/tanstack-query*.
set -euo pipefail
...
OUT="$FAMILIA/tanstack-query/references/mapa-de-ids.md"
...

{
  ...
  echo "> Regenerate with \`bash skills/tanstack/tanstack-query/scripts/gerar-mapa-de-ids.sh\`."
  echo
...
echo "written: $OUT ($(grep -c '^| `TSQ' "$OUT") IDs)"
