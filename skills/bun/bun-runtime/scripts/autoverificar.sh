...
"${RG[@]}" 'process\.env\.' "$DIR" | head -8 || vazio

titulo "5. Espera por tempo em código de produção"
"${RG[@]}" 'Bun\.sleep\(|setTimeout\(' "$DIR" | head -6 || vazio
...
rg -n --no-messages '\-\-watch|\-\-hot' package.json 2>/dev/null | sed 's|^|   |' || vazio
...
rg -n --no-messages 'bun run [a-z:-]+ --[a-z]' package.json .github/workflows/*.y*ml 2>/dev/null | sed 's|^|   |' || vazio
