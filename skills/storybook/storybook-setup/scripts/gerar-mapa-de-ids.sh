...
{
  echo "---"
  echo "gerado-por: plugins/hermes-frontend/skills/storybook-setup/scripts/gerar-mapa-de-ids.sh"
  ...
  echo "> Regenerar com \`bash plugins/hermes-frontend/skills/storybook-setup/scripts/gerar-mapa-de-ids.sh\`."
  echo
...
for s in setup story test; do
  cp "$TMP" "$PLUGIN/skills/storybook-$s/references/mapa-de-ids.md"
done
rm -f "$TMP"
echo "gerado nas 3 skills ($(grep -c '^| `SB' "$PLUGIN/skills/storybook-setup/references/mapa-de-ids.md") IDs)"
