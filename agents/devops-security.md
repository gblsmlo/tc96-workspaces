...
| CI workflow, credentials in Actions | [Github Actions](../knowledge-base/github-actions.md) · · | — |
...
**Integration and delivery** — main always ready to deploy; incomplete code goes behind a flag; the practice swaps gates for tests and demands maturity. DORA metrics as a reading of the pipeline:. Preview per PR:.

**CI** — (OIDC, not a static key). What monorepo CI needs: [Monorepo com Bun - estrutura e tooling](../knowledge-base/monorepo-com-bun-estrutura-e-tooling.md) § 7. Test pipeline: [Bun - Testes - Cobertura e CI](../knowledge-base/bun-testes-cobertura-e-ci.md), [Playwright - Execução, Retries e CI](../knowledge-base/playwright-execucao-retries-e-ci.md).

**Secrets and configuration** — `.env` is a local convenience; the secret is loaded at startup, never per request; sidecar when the application should not know the manager; least privilege per path; rotation. AWS: when rotation is a requirement for the rest — which does not replace a secrets manager. Boot-time validation is the code's (`ZOD-ENV-*`); what arrives there is this agent's.

**Deploy** —; network:; static frontend:;. A Bun container ends with `process.exit` after draining (`BUN-SYS-11`).
