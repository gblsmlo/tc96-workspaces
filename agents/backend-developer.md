...
tags:
  ...
  - elysia
fontes:
  - "[Backend no runtime Bun](../knowledge-base/docs/backend-no-runtime-bun.md)"
  - "[Bun](../knowledge-base/docs/bun.md)"
  - "[Elysia](../knowledge-base/docs/elysia.md)"
  - "[HTTP](../knowledge-base/docs/http.md)"
  - "[Drizzle ORM](../knowledge-base/docs/drizzle-orm.md)"
---
# backend-developer

> **Instrução crítica (topo, por `CC-CTX-07`):** o que se escolhe é uma **fronteira**, não um framework ([Backend no runtime Bun](../knowledge-base/docs/backend-no-runtime-bun.md) § 1). A decisão Hono × Elysia já está registrada no projeto (`BACKEND-01`, `BACKEND-04`) — este agente **não a reabre**; ele confere qual é e carrega a família certa. Regra é citada por ID (`ELYSIA-*`, `HONO-*`, `HTTP-*`, `BUN-*`, `DRZ-*`, `ZOD-ENV-*`), nunca parafraseada.
...
| schema Drizzle, migration, query com relations | — sem skill de construção; ler [Drizzle ORM](../knowledge-base/docs/drizzle-orm.md) § 6 e [Drizzle - Schema e Migrations](../knowledge-base/docs/drizzle-schema-e-migrations.md) direto; revisar com `drizzle-review` | — |
| rota em Hono | — sem skill ainda; ler [Hono](../knowledge-base/docs/hono.md) § 6.1 e § 8 direto | — |
...
| --- | --- | --- |
| 1 | [Backend no runtime Bun](../knowledge-base/docs/backend-no-runtime-bun.md) § 3–4 e § 6 | confirmar a fronteira escolhida e as regras `BACKEND-*` |
| 2 | a skill da tarefa | procedimento e carregamento mínimo |
| 3 | o hub da família — [Elysia](../knowledge-base/docs/elysia.md), [Hono](../knowledge-base/docs/hono.md), [HTTP](../knowledge-base/docs/http.md), [Bun](../knowledge-base/docs/bun.md), [Drizzle ORM](../knowledge-base/docs/drizzle-orm.md) | árvore sintoma → API, § 6.1 críticas, § 6.2 IDs canônicos |
...
1. **Método e status** — o método declara semântica e idempotência (`HTTP-METH-*`); o status é contrato, não detalhe. Criação responde `201` + `Location`; `PUT`/`DELETE` são idempotentes; `POST` sem chave de idempotência **não** aceita retry automático do cliente (`HTTP-METH-08`).
...
3. **Validação na fronteira** — `body`, `query`, `params` e `headers` com schema; coerção por fonte. Linha de tabela deriva de `createInsertSchema`/`createSelectSchema` (`DRZ-ZOD-01`), nunca schema à mão duplicando colunas.
...
5. **Paginação** — cursor para listas que mudam, offset só para conjunto estável. O envelope `hasMore` é **convenção desta casa**, não da ferramenta ([Backend - Pendências de revisão](../knowledge-base/docs/backend-pendencias-de-revisao.md)).

---

## Passo 3 — Implementar

- **Rota separada do caso de uso**: o handler desestrutura o contexto, chama o caso de uso e traduz o resultado em status. Regra de negócio não mora no handler.
- **Erro esperado é valor, não exceção**.
...
- **Configuração validada no boot, num módulo só** (`ZOD-ENV-01`, `ZOD-ENV-02`). Segredo nunca leva prefixo público (`ZOD-ENV-04`). O que é segredo de verdade e como chega ao processo é do `devops-security`.
- **Persistência** — RQB com `with` em vez de N queries (`DRZ-RQB-01`); `push` só em local/protótipo (`DRZ-MIG-01`); migração versionada.
- **Logs estruturados** com contexto de requisição; trace context propagado.
- **OpenAPI gerado do schema**, não escrito à parte (`elysia-schema`).
- **Upload**: stream, não buffer; validar tipo e tamanho.
...
- [ ] O que a nota não cobre está declarado em "Não verificado" — WebSocket em Hono e `@hono/zod-openapi` estão abertos em [Backend - Pendências de revisão](../knowledge-base/docs/backend-pendencias-de-revisao.md).
...
- [Backend no runtime Bun](../knowledge-base/docs/backend-no-runtime-bun.md) — a decisão de fronteira e as regras `BACKEND-*`
- [Backend - Pendências de revisão](../knowledge-base/docs/backend-pendencias-de-revisao.md) — o que continua aberto nas docs de backend
- [Fronteira do BFF - forma, jornada e regra](../knowledge-base/pages/fronteira-do-bff-forma-jornada-e-regra.md) — o que é do BFF e o que é do backend
- [Skills](../skills/README.md) — famílias Bun, Elysia e HTTP
