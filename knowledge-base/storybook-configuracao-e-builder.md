...
O desenho está em [Monorepo com Bun - estrutura e tooling](monorepo-com-bun-estrutura-e-tooling.md): `apps/storybook` é app deployável e **folha do grafo** — ele consome `apps/web` e `packages/ui`, e ninguém consome ele. É isso que permite tirá-lo do build de produção sem tocar em nenhum outro pacote (`SB-CFG-07`).
...
O conserto é separar a augmentation num módulo próprio (`router.ts`), exportá-lo, e importar **de tipo** onde precisar. Ver [Monorepo com Bun - estrutura e tooling](monorepo-com-bun-estrutura-e-tooling.md) § 5.7, onde isso foi verificado por sabotagem.
...
- [Monorepo com Bun - estrutura e tooling](monorepo-com-bun-estrutura-e-tooling.md) — o grafo de dependências e a armadilha de augmentation
