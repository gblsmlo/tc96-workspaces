...
Não é duplicação a eliminar: são níveis diferentes. E há um detalhe operacional em CI — o `--filter` do Bun não alcança o segundo, e filtro negado não funciona no Bun (ver [Monorepo com Bun - estrutura e tooling](monorepo-com-bun-estrutura-e-tooling.md) § 4). Os dois comandos são invocados separadamente.
...
- [Monorepo com Bun - estrutura e tooling](monorepo-com-bun-estrutura-e-tooling.md) — o comportamento de `--filter` em CI
