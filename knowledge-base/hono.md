...
```
SEMPRE:   Hono.md § 2 (modelo mental)
          Hono.md § 5 (árvores de decisão)
          Hono.md § 6 + § 6.1 (regras normativas e críticas)

AO DECLARAR ROTA ou ler/escrever no Context:
          Hono - Roteamento e Contexto.md

AO TOCAR auth, CORS, log, cache, timeout, headers:
          Hono - Middleware e Ciclo de Vida.md

AO RECEBER INPUT do request, ou quando o tipo tiver que chegar no frontend:
          Hono - Validação e RPC.md

EM CONSUMO PELO REACT (queryFn, mutationFn, hc, formulário, cancelamento):
          Hono - Validação e RPC.md § 6.1   ← a ponte normativa
          Hono.md § 8                       ← o panorama e o critério de adoção
          Hono - Middleware e Ciclo de Vida.md § 5  ← CORS: SPA + API é
...
4. **Validar na fronteira, sempre.** Nenhum handler lê `c.req.json()` cru para alimentar domínio (`HONO-RPC-01`)..
...
**`signal` atravessa.** O segundo argumento de `$get`/`$post` aceita `{ init: RequestInit }`, e a doc mostra `signal` como caso de uso. Passar o `signal` que o TanStack Query fornece na `queryFn` faz o cancelamento chegar até a rede.
