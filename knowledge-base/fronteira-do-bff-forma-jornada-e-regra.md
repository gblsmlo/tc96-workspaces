...
Como a nota irmã, esta é **normativa**: tem IDs (`BFF-*`), invariantes e contrato de skill. Não
resume documentação de uma ferramenta — registra uma decisão desta casa. Ver a exceção explicada em
[Architecture in React](architecture-in-react.md) § 5.
...
- — a camada de fora
- ·
- [Elysia](elysia.md) · [Elysia - Schema e Eden](elysia-schema-e-eden.md) — a ferramenta que torna `BFF-07` verificável
- [TanStack Query - Padrões de Consulta](tanstack-query-padroes-de-consulta.md) — leque e waterfall pelo lado do cliente
...
ferramenta foram verificadas rodando — quando divergirem da doc verificada da ferramenta, a doc vence.
