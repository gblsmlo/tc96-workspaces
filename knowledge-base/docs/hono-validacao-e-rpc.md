...
**`z.coerce` é o que reconcilia HTTP com tipos.** `query`, `param` e `header` chegam sempre como `string`. `z.coerce.number` converte no servidor — e o cliente RPC continua obrigado a enviar `string` (§ 5). O schema é a fronteira entre os dois mundos; ver.
...
**4. Duas pré-condições de projeto.** Versão de `hono` **idêntica** no servidor e no cliente — divergência produz o erro *"Type instantiation is excessively deep and possibly infinite"*, que a doc cita nominalmente. E `"strict": true` no `compilerOptions` dos **dois** `tsconfig.json` (a doc aponta a issue #2270). Ver.
