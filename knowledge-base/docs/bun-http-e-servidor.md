...
**Observabilidade embutida:** `server.pendingRequests`, `server.pendingWebSockets`, `server.subscriberCount(topico)`, `server.requestIP(req)` (retorna `null` para socket Unix). É o mínimo para um `/metrics`.
