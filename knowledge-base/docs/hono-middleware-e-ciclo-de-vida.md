...
`requestId` antes de `logger` é o que permite correlacionar entrada e saída da mesma requisição — ver e. E `logger` da caixa imprime uma linha por request: útil em desenvolvimento, insuficiente para produção estruturada e fácil de virar ruído —.
...
`origin` aceita `string`, `string[]` ou `(origin, c) => string`; `allowMethods` também aceita função de `(origin, c)`. E ao rodar Hono sob Vite, a doc pede `server.cors: false` no `vite.config.ts` para o CORS embutido do Vite não conflitar com o middleware. Ver.

**JWT: `secret` é segredo de inicialização.** `jwt({ secret: c.env.JWT_SECRET })` exige o wrapper da § 6, e o segredo vem do ambiente, não do código — ver e `Arquivos.env não substituem secret management`. Para chave assimétrica e rotação, o caminho é JWKS; Hono tem um middleware JWK dedicado, **não verificado nesta doc**. Autorização — quem pode o quê — não é o middleware de JWT: ele só prova identidade. Ver.
...
`routePath(c)` aceita um índice opcional (`routePath(c, 0)` = primeira rota casada, `routePath(c, -1)` = última), útil quando há middleware curinga no caminho. Ver.
