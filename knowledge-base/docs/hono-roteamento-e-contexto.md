...
**Status literal não é cosmético.** `c.json({ pedido }, 200)` e `c.json({ pedido })` produzem a mesma resposta e **tipos diferentes**: só o primeiro permite `InferResponseType<typeof $get, 200>` e a discriminação por `res.status` no cliente. Em rota consumida pelo frontend, o status é parte do contrato — ver.
