...
**roteia**; o conteúdo mora nos Zettels e nas outras notas da knowledge-base.
...
| Referência de API | [React - Estado e Reatividade](react-estado-e-reatividade.md) |
...
| Fronteira dentro do componente | [React - Patterns](react-patterns.md) § 6 |
...
| Rota, loader e busca de dados | [TanStack Router](tanstack-router.md) |
...
| Assincronia e estados de carregamento | [React - Suspense e Assincronia](react-suspense-e-assincronia.md) |
...
| Regras normativas de React | [React - Rules of React](react-rules-of-react.md) |
...
REGRA NORMATIVA DE REACT
  → [React - Rules of React](react-rules-of-react.md) e [React.js](react-js.md) § 6

CONSULTA DE API
  → [React.js](react-js.md) § 4 (mapa da API) → o satélite indicado
```

A hierarquia de fontes da knowledge-base, da mais forte para a mais fraca: documentação de
ferramenta verificada (`verificado-em` no frontmatter) → Zettels (ideias consolidadas) → páginas-
roteador, como esta, que só apontam e não têm ID. Divergência entre uma página-roteador e uma doc
verificada é bug da página. Ver [React.js](react-js.md) § 7, e `react-build` como exemplo de skill
que implementa esse contrato.

**Três exceções, explícitas.** [Feature-Based Architecture](feature-based-architecture.md),
[Fronteira do BFF - forma, jornada e regra](fronteira-do-bff-forma-jornada-e-regra.md) e [Monorepo com Bun - estrutura e tooling](monorepo-com-bun-estrutura-e-tooling.md) são
páginas **normativas**: têm IDs citáveis (`REACT-ARCH-*`, `BFF-*` e `MONO-*`), invariantes e
contrato de skill — diferente desta página, que só roteia. A diferença não é de onde o arquivo
mora: é que elas não resumem documentação externa de uma ferramenta, e sim registram uma decisão
desta casa, com convenções que só existem neste projeto. Uma página com contrato de skill é citável
como regra; esta página, que só roteia, não é. Quando uma doc de ferramenta contradisser uma dessas
três em fato verificável — comportamento do Biome, API do TanStack — a doc vence e a página
normativa é corrigida.
...
- [React.js](react-js.md) — hub de React: modelo mental, mapa da API, contrato de skill
- [React - Patterns](react-patterns.md) — decisão estrutural dentro do componente
