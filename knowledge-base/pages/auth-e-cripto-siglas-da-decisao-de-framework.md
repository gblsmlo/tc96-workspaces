...
| **JWT** | *JSON Web Token* | o token em si: claims JSON assinadas | [RFC 7519](https://www.rfc-editor.org/rfc/rfc7519) · [RFC 8725 - JWT Best Current Practices](../docs/rfc-8725-jwt-best-current-practices.md) |
| **JWS** | *JSON Web Signature* | o mecanismo de assinatura por trás do JWT | [RFC 7515](https://www.rfc-editor.org/rfc/rfc7515) |
| **PKCE** | *Proof Key for Code Exchange* | prova que quem troca o `code` é quem iniciou o fluxo, sem client secret | [RFC 7636](https://www.rfc-editor.org/rfc/rfc7636) · [RFC 9700 - OAuth 2.0 Security BCP](../docs/rfc-9700-oauth-2-0-security-bcp.md) §2.1.1 |
| **state** | — | valor opaco que atravessa o redirect e volta; defesa de CSRF do fluxo OAuth | [RFC 6749](https://www.rfc-editor.org/rfc/rfc6749) §10.12 · [RFC 9700 - OAuth 2.0 Security BCP](../docs/rfc-9700-oauth-2-0-security-bcp.md) §2.1 |
...
| **BFF** | *Backend For Frontend* | o servidor que guarda os tokens e fala com o browser só por cookie | [OAuth 2.0 for Browser-Based Applications](../docs/oauth-2-0-for-browser-based-applications.md) §6.1 |
| **RBAC** | *Role-Based Access Control* | acesso por role, não por identidade | [NIST RBAC - ANSI INCITS 359](../docs/nist-rbac-ansi-incits-359.md) · [WorkOS - RBAC](../docs/workos-rbac.md) |
| **CSRF** | *Cross-Site Request Forgery* | site terceiro dispara request autenticada usando seu cookie | [OWASP - Sessão e Autorização](../docs/owasp-sessao-e-autorizacao.md) |
...
> A [RFC 8725 - JWT Best Current Practices](../docs/rfc-8725-jwt-best-current-practices.md) §3.9 pede validação de *audience*. O token
> publicado não tem `aud` — a separação de público sai de `iss` **+** `client_id`. Ver
> [WorkOS - AuthKit](../docs/workos-authkit.md) #5. Access token — claims.
...
Lista completa e a armadilha (tratar ausência de `code` como `400` genérico engole esses erros): [WorkOS - AuthKit](../docs/workos-authkit.md) #11.3 Erros na geração da URL. É contrato do provedor — nenhum framework o muda.
...
"HMAC de state" é a segunda: o `state` carrega o destino pretendido (o `returnTo`) e uma tag que prova que ninguém o reescreveu. É o que permite ser stateless **sem** abrir open redirect — mas note que HMAC prova *integridade*, não *pertencimento*: continua sendo necessário amarrar o `state` ao browser daquele usuário e manter allowlist de rotas internas ([WorkOS - AuthKit](../docs/workos-authkit.md) #11.2 state é responsabilidade sua).
...
O racional já está registrado em [WorkOS - AuthKit](../docs/workos-authkit.md) #14. Notas de arquitetura — bondingAI: a camada de identidade é *pooled* mesmo com infraestrutura *siloed*, então o `org_id` é **asserção a ser conferida, não fonte de verdade**. Sem o probe, um token legítimo de outro tenant é aceito por um silo que não é o dele — a falha de autorização por *object/tenant* que a literatura chama de **IDOR** em nível de tenant, e que a OWASP classifica em *Broken Object Level Authorization*.
...
- [WorkOS - AuthKit](../docs/workos-authkit.md) — a implementação concreta de tudo em §3
- [WorkOS - RBAC](../docs/workos-rbac.md) — onde `role`/`permissions` do shape da sessão são consumidos
- [OAuth 2.0 for Browser-Based Applications](../docs/oauth-2-0-for-browser-based-applications.md) — o padrão BFF que justifica o proxy
- [RFC 9700 - OAuth 2.0 Security BCP](../docs/rfc-9700-oauth-2-0-security-bcp.md) — PKCE, `state`, redirect URI exato
- [RFC 8725 - JWT Best Current Practices](../docs/rfc-8725-jwt-best-current-practices.md) — verificação de JWT e a lacuna do `aud`
- [RFC 6265 - Cookies HTTP](../docs/rfc-6265-cookies-http.md) — teto de 4KB e atributos do cookie selado
- [OWASP - Sessão e Autorização](../docs/owasp-sessao-e-autorizacao.md) — checklist de revisão
- [NIST RBAC - ANSI INCITS 359](../docs/nist-rbac-ansi-incits-359.md) — modelo formal de roles
