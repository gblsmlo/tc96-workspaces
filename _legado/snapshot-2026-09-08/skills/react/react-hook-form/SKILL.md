...
description: Trabalhar com formulários no React Hook Form — triar se RHF é a ferramenta, conectar campos, validar com Zod, campos condicionais e listas dinâmicas, submeter, e diagnosticar re-render e campo que não envia, citando IDs `RHF-*`, com doze sondas executáveis — use quando a tarefa for montar formulário novo, integrar componente de UI controlado por Controller, mapear erro de servidor para o campo, usar useFieldArray, decidir quem é dono do estado de submissão, ou revisar um formulário existente. Não use para um campo isolado sem validação, que são as Actions nativas via react-developer, para estado de servidor, que é tanstack-query, nem para estado que pertence à URL, que é tanstack-router.
tags:
  - skill
  - react
  - react-hook-form
fonte: "[[React Hook Form]]"
---

# react-hook-form

> **Fonte desta skill:** [[React Hook Form]] e seus três satélites em `Docs/`.
...
Contrato que esta skill implementa: [[React Hook Form]] § 7. As invariantes de lá valem em toda tarefa, sem repetição por seção.
...
Três notas convergem nesse corte — [[React Hook Form]] § 5.4, [[React - Formulários e Actions]] § 6 e [[React - Patterns]] § 4 —, então não é preferência de estilo. **Registre a decisão em uma frase:** se você não consegue nomear qual dos cinco gatilhos se aplica, RHF é dependência sem contrapartida.

---

## Carregamento mínimo

Conforme [[React Hook Form]] § 7:
...
Abaixo, os satélites aparecem pelo nome curto: **Registro**, **Validação**, **Estado**; **hub** é [[React Hook Form]].
...
```bash
bash ~/.claude/skills/react-hook-form/scripts/sondas.sh src
```
...
- [[React Hook Form]] — hub, modelo mental, árvores de decisão, contrato de skill (fonte)
- [[React Hook Form - Registro e Controle]] · [[React Hook Form - Validação e Resolvers]] · [[React Hook Form - Estado e Performance]] — satélites, um por vez
- [[React - Formulários e Actions]] — a alternativa nativa, que o Passo 0 pode indicar
- [[react-developer]] · [[react-review]] · [[react-structure]] · [[tanstack-query]] · [[tanstack-router]] — skills vizinhas
- Zettels: [[React Hook Form e Zod separam captura e validação]] · [[Zod como schema de runtime]] · [[Tratamento de erros esperados e inesperados]] · [[Server Actions são fronteiras de confiança]] · [[Atualizações otimistas exigem snapshot e rollback]]
